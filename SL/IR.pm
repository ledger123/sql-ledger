        ...
        if ( $form->{groupprojectnumber} || $form->{grouppartsgroup} ) {
            if ( $item->[1] ne $sameitem ) {
                ...
                if ($ok) {
                    ...
                    for (
                        qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sell sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode lot expires make model)
                      )
                    {
                        push( @{ $form->{$_} }, "" );
                    }
            ...
            if ($subtotal) {
                if ( $j < $k ) {
                    ...
                    if ( $sortlist[$j]->[1] ne $sameitem ) {
                        ...
                        for (
                            qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sell sellprice listprice netprice discount discountrate itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode lot expires make model)
                          )
                        {
                            push( @{ $form->{$_} }, "" );
                        }
        ...
        if ( $form->{"qty_$i"} ) {
		  ...
		  WHERE id = $id|;
            ...
            if ( $form->{"inventory_accno_id_$i"} ) {
                ...
                # update parts table
                $form->update_balance( $dbh, "parts", "onhand", qq|id = $form->{"id_$i"}|, $form->{"qty_$i"} ) unless $form->{shipped};
    ...
    for $i ( 1 .. $form->{paidaccounts} ) {
        ...
        if ( $form->{"paid_$i"} ) {
			...
			$form->{id}, $voucherid, | . $dbh->quote( $form->{voucher}{payment}{$voucherid}{vouchernumber} ) . qq|)|;
                        ...
                        $form->update_balance( $dbh, 'br', 'amount', qq|id = $form->{voucher}{payment}{$voucherid}{br_id}|, $amount * -1 );
                    }
                }
            }
            ...
            $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, transdate,
              ...
              . $dbh->quote( $form->{"source_$i"} ) . qq|, | . $dbh->quote( $form->{"memo_$i"} ) . qq|, $cleared,
...
sub process_kit {
    ...
    while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        if ( $ref->{inventory_accno_id} ) {
            ...
            # update onhand
            $form->update_balance( $dbh, "parts", "onhand", qq|id = $ref->{parts_id}|, $ref->{qty} * $form->{"qty_$i"} ) unless $form->{shipped};
        }
...
sub reverse_invoice {
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        if ( $ref->{allocated} > 0 ) {
            ...
            while ( $pref = $sth->fetchrow_hashref(NAME_lc) ) {
                ...
                #adjust allocated
                $form->update_balance( $dbh, "invoice", "allocated", qq|id = $pref->{id}|, $qty );
            ...
            while ( $pref = $sth->fetchrow_hashref(NAME_lc) ) {
                ...
                #adjust allocated
                $form->update_balance( $dbh, "invoice", "allocated", qq|id = $pref->{id}|, $qty * -1 );
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        $form->update_balance( $dbh, 'br', 'amount', qq|id = $ref->{br_id}|, $amount * -1 );
    }
    $sth->finish;
