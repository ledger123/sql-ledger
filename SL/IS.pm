...
sub process_kit {
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        if ( $ref->{inventory_accno_id} ) {
            ...
            # update onhand
            $form->update_balance( $dbh, "parts", "onhand", qq|id = $ref->{parts_id}|, $ref->{qty} * $form->{"qty_$i"} * -1 ) unless $form->{shipped};
        }
...
sub reverse_invoice {
    ...
    while ( $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        $form->update_balance( $dbh, 'br', 'amount', qq|id = $ref->{br_id}|, $amount );
    }
    $sth->finish;
